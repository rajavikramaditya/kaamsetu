...
export default function RequestSuccessPage() {
  ...
  const trackUrl = `/track?open=${encodeURIComponent(session.job_ref ?? "")}`;
  ...
  return (
    <CustomerShell title="Request Submitted" active="request">
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-center">
        <p className="text-sm font-medium text-amber-900">Track code (backup for other phones)</p>
        <p className="mt-3 font-mono text-3xl font-bold tracking-[0.3em] text-stone-900">
          {session.track_code}
        </p>
        <p className="mt-3 text-xs text-amber-900">
          Saved on this device. On another phone, use job reference + track code.
        </p>
      </div>

      <p className="rounded-xl bg-teal-50 px-4 py-3 text-sm text-teal-900">
        This request is saved on this device. Tap <strong>View this request</strong> anytime
        to check status without re-entering codes.
      </p>
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          Add photos or voice note (optional)
        </NextLink>
      </div>
    </CustomerShell>
  );
