...
import { patchCustomerSession } from "@/lib/customer/session";
...
export default function TrackJobPage() {
  ...
  useEffect(() => {
    ...
    setShowManualForm(saved.length === 0);

    const openRef = new URLSearchParams(window.location.search).get("open");
    if (openRef) {
      const normalized = openRef.trim().toUpperCase();
      const request = findSavedRequest(normalized) ?? saved.find((r) => r.job_ref === normalized);
      if (request) {
        void openSavedRequest(request);
      }
    }
  }, [openSavedRequest]);
    ...
    <CustomerShell title="My Requests" active="track">
        ...
        <section className="flex flex-col gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-stone-500">
              Tap a saved request to view status — no codes needed on this device.
            </p>
          </div>
      ...
      <section className="flex flex-col gap-3 border-t border-stone-200 pt-4">
        {savedRequests.length > 0 ? (
          ...
          <div>
            <h2 className="text-sm font-semibold text-stone-900">Track a request</h2>
            <p className="mt-1 text-sm text-stone-600">
              Enter your job reference, mobile number, and track code.
            </p>
          </div>
        )}

        {(showManualForm || savedRequests.length === 0) && (
      ...
      {error && !showManualForm && savedRequests.length > 0 && (
        <p className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-800">{error}</p>
      )}
